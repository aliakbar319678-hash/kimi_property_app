...
router.get('/vendor/wallet', authenticate, requireRole('vendor'), async (req: AuthRequest, res, next) => {
  try {
...
router.post('/invoices', authenticate, requireRole('vendor'), async (req: AuthRequest, res, next) => {
  try {
    const invoice = await FinanceService.generateInvoice(req.user!.id, req.body.workOrderId, req.body.items);
...
router.post('/payments/retry/:id', adminOrJwtAuth, async (req: AuthRequest, res, next) => {
  try {
    ...
    await query("UPDATE transactions SET status = 'completed', updated_at = NOW() WHERE id = $1 AND status = 'failed'", [txId]);
    res.json({ success: true, message: 'Transaction retried successfully' });
  } catch (e) { next(e); }
});
...
router.get('/export', adminOrJwtAuth, async (req: AuthRequest, res, next) => {
  try {
    const txns = await query("SELECT * FROM transactions ORDER BY created_at DESC");
    // Send mock CSV response
    let csv = 'ID,Amount,Status,Date\n';
    txns.rows.forEach((t: any) => {
      csv += `${t.id},${t.amount},${t.status},${t.created_at}\n`;
    });
