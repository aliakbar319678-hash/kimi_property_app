  ...
  try {
    ...
    const payoutRes = await query(`
      SELECT p.*, u.stripe_account_id 
       FROM payouts p
       JOIN users u ON p.payee_id = u.id
       WHERE p.id = $1 AND p.status IN ('queued', 'failed', 'pending')
    `, [payoutId]);
    ...
    if (!payout.stripe_account_id) {
      return res.status(400).json({ success: false, message: 'Payee has no connected Stripe account' });
    }
    ...
    try {
      const transfer = await StripeService.createTransfer(
        ...
        `Vendor Payout ID: ${payout.id}`
      );
      transferId = transfer.id;
    } catch (err: any) {
      return res.status(400).json({ success: false, message: `Stripe transfer failed: ${err.message}` });
    }

    // Update the transfer ID and status
    await query(
      `UPDATE payouts SET status = 'paid', processed_date = NOW(), updated_at = NOW(), stripe_transfer_id = $1 WHERE id = $2`,
      [transferId, payoutId]
    );
...
router.post('/process-batch', adminOrJwtAuth, async (req: any, res, next) => {
  try {
    const payoutsRes = await query(`
      SELECT p.id, p.amount, p.payee_id, u.stripe_account_id 
      FROM payouts p
      JOIN users u ON p.payee_id = u.id
    ...
    for (const payout of payoutsRes.rows) {
      if (!payout.stripe_account_id) {
        failCount++;
        errors.push(`Payout ${payout.id}: payee has no Stripe account`);
        await query(
