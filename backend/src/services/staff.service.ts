  ...
  static async list(page: number = 1, limit: number = 20) {
    ...
    const res = await query(
      `SELECT
         ...
         (
           (SELECT COUNT(*) FROM tickets t WHERE t.assigned_staff_id = u.id AND t.status NOT IN ('resolved','closed','cancelled')) +
           (SELECT COUNT(*) FROM support_tickets st WHERE st.assigned_to LIKE u.display_name || '%' AND st.status NOT IN ('resolved','closed','cancelled'))
